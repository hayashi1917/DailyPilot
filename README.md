...
- カレンダービューで予定がある日を「・」表示
- AIによる1週間の振り返り生成（OpenAI APIキー未設定時はローカル要約）
- 振り返りと明日のスケジュール記入のブラウザ通知リマインド
- Googleカレンダー連携
  - Google OAuth による接続
...
| `OPENAI_API_KEY` | 任意 | AIによる1週間の振り返り生成に使うOpenAI APIキー。未設定時はローカル要約を表示します | `sk-...` |
| `OPENAI_MODEL` | 任意 | 週次振り返り生成に使うOpenAIモデル | `gpt-4.1-mini` |
